...
export class FaceRecognitionService {
  ...
  public static readonly MATCH_THRESHOLD = 0.18;
 ...
 public static async initModel(): Promise<void> {
  ...
  try {
    // Copy model from assets to cache dir for file:// access
    const destPath = `${RNFS.CachesDirectoryPath}/mobilefacenet.tflite`;
    const exists = await RNFS.exists(destPath);
    if (!exists) {
      await RNFS.copyFileAssets('mobilefacenet.tflite', destPath);
    }
